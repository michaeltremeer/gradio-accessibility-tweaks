...
To develop with the repository and test changes to the front-end code, follow the instructions below. Building the front-end requires a bash terminal, so using WSL is recommended.

1. Create a new python 3.11 virtual environment and activate it.

Option A - using Conda:

```
conda create -n gradio-accessibility-test python=3.11 --no-default-packages
conda activate gradio-accessibility-test
```
...
```
pyenv install 3.11
pyenv virtualenv 3.11 gradio-accessibility-test
pyenv activate gradio-accessibility-test
```

Option C - using venv (requires running this code from an existing python 3.11 environment).
...
2. Install the external dependencies and then this custom gradio package (in editable mode, so changes to the code are reflected immediately).

```
pip install -r accessibility_requirements.txt
pip install -e .
```

3. Make changes to the front-end code in the `js` directory.

4. Rebuild the front-end using a bash terminal:

```
bash scripts/build_frontend.sh
```

5. Run the test gradio server to test the changes:

```
python accessibility_test_app.py
```

6. Navigate to <http://localhost:8000> in your browser to view the test app.

Each time you make changes to the front-end code, you will need to rebuild the front-end and run the restart the gradio server to load the new front-end changes.
